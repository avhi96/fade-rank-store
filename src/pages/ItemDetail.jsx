        ...
        try {
            if (liked) {
                ...
                await setDoc(ref, {
                    ...
                    name: item.name,
                    price: item.price,
                    image: item.images?.[0] || item.image || '/placeholder.jpg', // force image
                    type, // important for navigation
                    timestamp: Date.now()
                });
                    ...
                    <div className="flex gap-2 mt-4 overflow-x-auto">
                        {[...(item.images || [])].map((img, i) => (
                            <img
                                ...
                                className={`h-20 w-20 object-cover rounded-md cursor-pointer border transition-all ${img === activeImage ? 'border-blue-500 scale-105' : 'border-gray-300'}`}
                                alt={`thumb-${i}`}
                            />
                        ))}
                    </div>
