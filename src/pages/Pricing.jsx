...
const Pricing = () => {
  ...
  try {
    ...
    // ✅ Email Notification
    emailjs.send('service_q2wb6k2', 'template_n6cpaob', {
      email: user.email,
      serviceTitle: selectedService.title,
      price: selectedService.price,
      ...
      <div className="max-w-7xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service) => (
            <div
              key={service.id}
              className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-xl p-6 shadow-md hover:shadow-lg transition-all flex flex-col items-center text-center"
            >
              <div className="mb-4">{service.icon}</div>
              <h2 className="text-xl font-bold text-gray-800 dark:text-white">{service.title}</h2>
